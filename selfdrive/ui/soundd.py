...
class Soundd:
  ...
  def soundd_thread(self):
    ...
    sm = messaging.SubMaster(['controlsState', 'microphone', 'catpilotPlan'])
    ...
    with self.get_stream(sd) as stream:
      ...
      while True:
        ...
        if sm.updated['microphone'] and self.current_alert == AudibleAlert.none: # only update volume filter when not playing alert
          ...
          if self.catpilot_toggles.alert_volume_controller:
            self.auto_volume = self.calculate_volume(float(self.spl_filter_weighted.x))
            self.current_volume = 0.0
          else:
            self.current_volume = self.calculate_volume(float(self.spl_filter_weighted.x))

        elif self.catpilot_toggles.alert_volume_controller and self.current_alert in self.volume_map:
        ...
        if sm['catpilotPlan'].togglesUpdated:
          self.catpilot_toggles = get_catpilot_toggles()
  ...
  def update_catpilot_sounds(self):
    self.volume_map = {
      AudibleAlert.engage: self.catpilot_toggles.engage_volume / 100.0,
      AudibleAlert.disengage: self.catpilot_toggles.disengage_volume / 100.0,
      AudibleAlert.refuse: self.catpilot_toggles.refuse_volume / 100.0,

      AudibleAlert.prompt: self.catpilot_toggles.prompt_volume / 100.0,
      AudibleAlert.promptRepeat: self.catpilot_toggles.prompt_volume / 100.0,
      AudibleAlert.promptDistracted: self.catpilot_toggles.promptDistracted_volume / 100.0,

      AudibleAlert.warningSoft: self.catpilot_toggles.warningSoft_volume / 100.0,
      AudibleAlert.warningImmediate: self.catpilot_toggles.warningImmediate_volume / 100.0,

      AudibleAlert.goat: self.catpilot_toggles.prompt_volume / 100.0,
      AudibleAlert.startup: self.catpilot_toggles.engage_volume / 100.0
    }
    ...
    if self.catpilot_toggles.sound_pack != "stock":
      self.sound_directory = ACTIVE_THEME_PATH / "sounds"
    else:
      self.sound_directory = Path(BASEDIR) / "selfdrive" / "assets" / "sounds"

    if self.catpilot_toggles.sound_pack != self.previous_sound_pack:
      self.load_sounds()
      self.previous_sound_pack = self.catpilot_toggles.sound_pack
