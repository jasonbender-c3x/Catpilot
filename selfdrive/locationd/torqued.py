...
class TorqueEstimator(ParameterEstimator):
  ...
  def get_msg(self, valid=True, with_points=False, catpilot_toggles=None):
    ...
    liveTorqueParameters.latAccelFactorFiltered = float(self.filtered_params['latAccelFactor'].x if not catpilot_toggles.use_custom_latAccelFactor else catpilot_toggles.latAccelFactor)
    liveTorqueParameters.latAccelOffsetFiltered = float(self.filtered_params['latAccelOffset'].x)
    liveTorqueParameters.frictionCoefficientFiltered = float(self.filtered_params['frictionCoefficient'].x if not catpilot_toggles.use_custom_friction else catpilot_toggles.friction)
...
def main(demo=False):
  ...
  sm = messaging.SubMaster(['carControl', 'carOutput', 'carState', 'liveLocationKalman', 'liveDelay', 'catpilotPlan'], poll='liveLocationKalman')
  ...
  if not catpilot_toggles.liveValid:
  ...
  while True:
    ...
    if sm.frame % 5 == 0:
      pm.send('liveTorqueParameters', estimator.get_msg(valid=sm.all_checks(), with_points=False, catpilot_toggles=catpilot_toggles))

    # Cache points every 60 seconds while onroad
    if sm.frame % 240 == 0:
      msg = estimator.get_msg(valid=sm.all_checks(), with_points=True, catpilot_toggles=catpilot_toggles)
