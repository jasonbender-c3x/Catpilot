...
class LateralLagEstimator:
  ...
  def get_msg(self, valid: bool, debug: bool = False, catpilot_toggles: SimpleNamespace = None) -> capnp._DynamicStructBuilder:
    ...
    if catpilot_toggles.use_custom_steerActuatorDelay:
      liveDelay.lateralDelay = catpilot_toggles.steerActuatorDelay
...
def main():
  ...
  sm = messaging.SubMaster(['livePose', 'liveCalibration', 'carState', 'controlsState', 'carControl', 'catpilotPlan'], poll='livePose')
  ...
  while True:
    ...
    if sm.frame % 5 == 0:
      ...
      lag_msg = lag_learner.get_msg(sm.all_checks(), DEBUG, catpilot_toggles)
