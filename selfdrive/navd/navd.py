...
class RouteEngine:
  ...
  def calculate_route(self, destination):
    ...
    try:
      ...
      if len(r['routes']):
        ...
        if self.catpilot_toggles.conditional_navigation_intersections:
          self.stop_signal = []
          self.stop_coord = []
  ...
  def send_instruction(self):
    ...
    fp_msg = messaging.new_message('catpilotNavigation', valid=True)

    if self.step_idx is None:
      msg.valid = False
      self.pm.send('navInstruction', msg)

      fp_msg.catpilotNavigation.navigationSpeedLimit = 0
      self.pm.send('catpilotNavigation', fp_msg)
      return
    ...
    if self.catpilot_toggles.conditional_navigation:
      ...
      if closest_condition_indices:
        ...
        self.approaching_intersection = self.catpilot_toggles.conditional_navigation_intersections and distance_to_condition < max((seconds_to_stop * v_ego), 25)
      else:
        self.approaching_intersection = False

      self.approaching_turn = self.catpilot_toggles.conditional_navigation_turns and distance_to_maneuver_along_geometry < max((seconds_to_stop * v_ego), 25)
    else:
      self.approaching_intersection = False
      self.approaching_turn = False

    fp_msg.catpilotNavigation.approachingIntersection = self.approaching_intersection
    fp_msg.catpilotNavigation.approachingTurn = self.approaching_turn
    fp_msg.catpilotNavigation.navigationSpeedLimit = self.nav_speed_limit
...
def main():
  pm = messaging.PubMaster(['navInstruction', 'navRoute', 'catpilotNavigation'])
  sm = messaging.SubMaster(['carState', 'liveLocationKalman', 'managerState', 'catpilotPlan'])
