  ...
  def update(self, CC, CS, now_nanos, catpilot_toggles):
    ...
    if self.CP.catpilotLongitudinalControl:
      ...
      if self.frame % 4 == 0:
        ...
        if catpilot_toggles.long_pitch and len(CC.orientationNED) > 1:
        ...
        if not CC.longActive:
          ...
          self.apply_gas = self.params.INACTIVE_REGEN
          self.apply_brake = int(min(-100 * catpilot_toggles.stopAccel, self.params.MAX_BRAKE))
        else:
          # Normal operation
        ...
        if self.CP.carFingerprint not in CC_ONLY_CAR:
          ...
          if CC.cruiseControl.resume and CS.pcm_acc_status == AccState.STANDSTILL and catpilot_toggles.volt_sng:
            acc_engaged = False
          else:
