...
def calculate_speed_limit(cp_cam, catpilot_toggles):
  speed_limit_unit = cp_cam.vl["RSA1"]["TSGN1"]
  speed_limit_value = cp_cam.vl["RSA1"]["SPDVAL1"]

  if speed_limit_unit == 1 and not catpilot_toggles.force_mph_dashboard:
    return speed_limit_value * CV.KPH_TO_MS
  elif speed_limit_unit == 36 or catpilot_toggles.force_mph_dashboard:
...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, CC, catpilot_toggles):
    ...
    ret.vEgoCluster = ret.vEgo * catpilot_toggles.cluster_offset
    ...
    fp_ret.dashboardSpeedLimit = calculate_speed_limit(cp_cam, catpilot_toggles)
    ...
    if self.FPCP.fpFlags & ToyotaCatPilotFlags.ZSS:
  ...
  def get_can_parser(CP, FPCP):
    ...
    if FPCP.fpFlags & ToyotaCatPilotFlags.ZSS:
