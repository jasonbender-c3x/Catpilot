...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos, catpilot_toggles):
    ...
    if CS.pcm_acc_status != 8 or catpilot_toggles.sng_hack:
    ...
    if self.CP.openpilotLongitudinalControl:
      if self.frame % 3 == 0:
        ...
        if catpilot_toggles.catsgomoo_tweak:
        ...
        if self.CP.flags & ToyotaFlags.SECOC.value:
          can_sends.append(toyotacan.create_accel_command(self.packer, 0, pcm_cancel_cmd, self.permit_braking, self.standstill_req, lead,
                                                          CS.acc_type, fcw_alert, self.distance_button, catpilot_toggles.reverse_cruise_increase))
      ...
      if pcm_cancel_cmd:
        if self.CP.carFingerprint in UNSUPPORTED_DSU_CAR:
          ...
          can_sends.append(toyotacan.create_accel_command(self.packer, 0, pcm_cancel_cmd, True, False, lead, CS.acc_type, False, self.distance_button, catpilot_toggles.reverse_cruise_increase))
