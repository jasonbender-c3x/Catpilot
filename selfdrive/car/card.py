...
class Car:
  ...
  def __init__(self, CI=None):
    ...
    self.sm = messaging.SubMaster(['pandaStates', 'carControl', 'liveCalibration', 'onroadEvents', 'catpilotPlan'])
    self.pm = messaging.PubMaster(['sendcan', 'carState', 'carParams', 'carOutput', 'catpilotCarState'])
    ...
    if CI is None:
      ...
      self.CI, self.CP, FPCP = get_car(self.can_sock, self.pm.sock['sendcan'], experimental_long_allowed, self.params, num_pandas, get_catpilot_toggles())
    else:
    ...
    # CatPilot variables
    self.catpilot_card = CatPilotCard(self)

    self.catpilot_toggles = get_catpilot_toggles()

    if self.catpilot_toggles.acceleration_profile == 3:
      self.CP.alternativeExperience |= ALTERNATIVE_EXPERIENCE.RAISE_LONGITUDINAL_LIMITS_TO_ISO_MAX

    if self.catpilot_toggles.always_on_lateral:
      self.CP.alternativeExperience |= ALTERNATIVE_EXPERIENCE.ALWAYS_ON_LATERAL
      self.CP.alternativeExperience |= ALTERNATIVE_EXPERIENCE.DISABLE_DISENGAGE_ON_GAS

    fpcp_bytes = FPCP.to_bytes()
    self.params.put("CatPilotCarParams", fpcp_bytes)
    self.params.put_nonblocking("CatPilotCarParamsPersistent", fpcp_bytes)
  ...
  def state_update(self) -> car.CarState:
    ...
    CS, FPCS = self.CI.update(self.CC_prev, can_strs, self.catpilot_toggles)
    ...
    # CatPilot variables
    FPCS = self.catpilot_card.update(CS, FPCS, self.sm)
  ...
  def state_publish(self, CS: car.CarState, FPCS: custom.CatPilotCarState):
    ...
    # catpilotCarState
    fpcs_send = messaging.new_message('catpilotCarState')
    fpcs_send.valid = CS.canValid
    fpcs_send.catpilotCarState = FPCS
    self.pm.send('catpilotCarState', fpcs_send)
  ...
  def controls_update(self, CS: car.CarState, CC: car.CarControl):
    ...
    if self.sm.all_alive(['carControl']):
      ...
      self.last_actuators_output, can_sends = self.CI.apply(CC, now_nanos, self.catpilot_toggles)
