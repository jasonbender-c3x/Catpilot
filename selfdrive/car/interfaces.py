...
class CarInterfaceBase(ABC):
  ...
  def apply(self, c: car.CarControl, now_nanos: int, catpilot_toggles) -> tuple[car.CarControl.Actuators, list[tuple[int, int, bytes, int]]]:
    return self.CC.update(c, self.CS, now_nanos, catpilot_toggles)
  ...
  @classmethod
  def get_params(cls, candidate: str, fingerprint: dict[int, dict[int, int]], car_fw: list[car.CarParams.CarFw], experimental_long: bool, catpilot_toggles: SimpleNamespace, params: Params, docs: bool):
    ...
    ret = cls._get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs, catpilot_toggles)
  ...
  @classmethod
  def get_catpilot_params(cls, candidate: str, fingerprint: dict[int, dict[int, int]], car_fw: list[car.CarParams.CarFw], catpilot_toggles: SimpleNamespace):
    fp_ret = custom.CatPilotCarParams.new_message()
      ...
      if platform in ChryslerCAR:
        if candidate == ChryslerCAR.RAM_HD_5TH_GEN:
          if 570 not in fingerprint[0]:
            fp_ret.fpFlags |= ChryslerCatPilotFlags.RAM_HD_ALT_BUTTONS.value
        ...
        if candidate in CANFD_CAR:
          ...
          if 0x1fa in fingerprint[CanBus(None, hda2, fingerprint).ECAN]:
            fp_ret.fpFlags |= HyundaiCatPilotFlags.NAV_MSG.value

          fp_ret.isHDA2 = hda2
        else:
          if 0x391 in fingerprint[0]:
            fp_ret.fpFlags |= HyundaiCatPilotFlags.CAN_LFA_BTN.value

          if 0x53E in fingerprint[2]:
            fp_ret.fpFlags |= HyundaiCatPilotFlags.LKAS12.value
  ...
  def _get_params(ret: car.CarParams, candidate, fingerprint: dict[int, dict[int, int]],
                  ...
                  catpilot_toggles: SimpleNamespace):
  ...
  def update(self, c: car.CarControl, can_strings: list[bytes], catpilot_toggles) -> car.CarState:
    # parse can
    ...
    # get CarState
    ret, fp_ret = self._update(c, catpilot_toggles)
  ...
  def create_common_events(self, cs_out, extra_gears=None, pcm_enable=True, allow_enable=True,
    ...
    for b in cs_out.buttonEvents:
      ...
      # CatPilot button presses
      if b.type == CatPilotButtonType.lkas and b.pressed:
