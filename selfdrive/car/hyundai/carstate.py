...
def calculate_speed_limit(CP, FPCP, cp, cp_cam):
  if CP.carFingerprint in CANFD_CAR:
    ...
    if FPCP.fpFlags & HyundaiCatPilotFlags.LKAS12:
      speed_limit = cp_cam.vl["LKAS12"]["CF_Lkas_TsrSpeed_Display_Clu"]
    else:
      speed_limit = 0

    if speed_limit in (0, 255) and FPCP.fpFlags & HyundaiCatPilotFlags.NAV_MSG:
...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam, catpilot_toggles):
    if self.CP.carFingerprint in CANFD_CAR:
      return self.update_canfd(cp, cp_cam, catpilot_toggles)

    ret = car.CarState.new_message()
    fp_ret = custom.CatPilotCarState.new_message()
    ...
    # CatPilot CarState functions
    fp_ret.brakeLights = bool(cp.vl["TCS13"]["BrakeLight"])

    if self.FPCP.fpFlags & HyundaiCatPilotFlags.LKAS12 or self.FPCP.fpFlags & HyundaiCatPilotFlags.NAV_MSG:
    ...
    if self.FPCP.fpFlags & HyundaiCatPilotFlags.CAN_LFA_BTN:
      self.lkas_enabled = cp.vl["BCM_PO_11"]["LFA_Pressed"]

    return ret, fp_ret

  def update_canfd(self, cp, cp_cam, catpilot_toggles):
    ret = car.CarState.new_message()
    fp_ret = custom.CatPilotCarState.new_message()
    ...
    # CatPilot CarState functions
    self.params = CarControllerParams(self.CP, ret.vEgoRaw, catpilot_toggles)

    fp_ret.brakeLights = bool(cp.vl["TCS"]["DriverBraking"])

    if self.FPCP.fpFlags & HyundaiCatPilotFlags.NAV_MSG:
  ...
  def get_can_parser(self, CP, FPCP):
    ...
    if FPCP.fpFlags & HyundaiCatPilotFlags.CAN_LFA_BTN:
      messages.append(("BCM_PO_11", 50))

    if FPCP.fpFlags & HyundaiCatPilotFlags.NAV_MSG:
  ...
  def get_can_parser_canfd(self, CP, FPCP):
    ...
    if CP.flags & HyundaiFlags.CANFD_HDA2 and FPCP.fpFlags & HyundaiCatPilotFlags.NAV_MSG:
  ...
  def get_cam_can_parser_canfd(CP, FPCP):
    ...
    if not (CP.flags & HyundaiFlags.CANFD_HDA2) and FPCP.fpFlags & HyundaiCatPilotFlags.NAV_MSG:
      messages.append(("CLUSTER_SPEED_LIMIT", 10))
