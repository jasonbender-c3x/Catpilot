...
def get_car(logcan, sendcan, experimental_long_allowed, params, num_pandas=1, catpilot_toggles=None):
  candidate, fingerprints, vin, car_fw, source, exact_match = fingerprint(logcan, sendcan, num_pandas)

  if candidate is None or catpilot_toggles.force_fingerprint:
    if catpilot_toggles.car_model is not None:
      candidate = catpilot_toggles.car_model
    else:
  ...
  if catpilot_toggles.block_user:
    candidate = "MOCK"

  CarInterface, _, _ = interfaces[candidate]
  CP = CarInterface.get_params(candidate, fingerprints, car_fw, experimental_long_allowed, catpilot_toggles, params, docs=False)
  FPCP = CarInterface.get_catpilot_params(candidate, fingerprints, car_fw, catpilot_toggles)
  CP.carVin = vin
  CP.carFw = car_fw
