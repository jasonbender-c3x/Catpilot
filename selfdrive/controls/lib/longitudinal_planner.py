...
class LongitudinalPlanner:
  ...
  def update(self, tomb_raider, sm, catpilot_toggles):
    if tomb_raider:
    ...
    v_cruise = sm['catpilotPlan'].vCruise
    ...
    if tomb_raider:
      if self.mode == 'acc':
        accel_limits = [sm['catpilotPlan'].minAcceleration, sm['catpilotPlan'].maxAcceleration]
    ...
    x, v, a, j, throttle_prob = self.parse_model(sm['modelV2'], self.v_model_error, v_ego, catpilot_toggles.taco_tune)
    ...
    self.mpc.set_weights(sm['catpilotPlan'].accelerationJerk, sm['catpilotPlan'].dangerJerk, sm['catpilotPlan'].speedJerk, prev_accel_constraint, personality=sm['controlsState'].personality)
    self.mpc.set_accel_limits(accel_limits_turns[0], accel_limits_turns[1])
    self.mpc.set_cur_state(self.v_desired_filter.x, self.a_desired)
    self.mpc.update(self.lead_one, self.lead_two, v_cruise, x, v, a, j, sm['catpilotPlan'].tFollow,
                    sm['catpilotPlan'].trackingLead, personality=sm['controlsState'].personality)
  ...
  def publish(self, classic_model, tomb_raider, sm, pm, catpilot_toggles):
    ...
    action_t = catpilot_toggles.longitudinalActuatorDelay + DT_MDL
    if tomb_raider:
      ...
      a_target, should_stop = get_accel_from_plan_classic(self.CP, longitudinalPlan.speeds, longitudinalPlan.accels, catpilot_toggles.longitudinalActuatorDelay, catpilot_toggles.vEgoStopping)
    else:
      a_target, should_stop = get_accel_from_plan(longitudinalPlan.speeds, longitudinalPlan.accels,
                                                  action_t=action_t, vEgoStopping=catpilot_toggles.vEgoStopping)
    longitudinalPlan.aTarget = float(a_target)
    longitudinalPlan.shouldStop = bool(should_stop) or sm['catpilotPlan'].forcingStopLength < 1
