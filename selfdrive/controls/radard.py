...
def get_lead(v_ego: float, ready: bool, tracks: dict[int, Track], lead_msg: capnp._DynamicStructReader,
             ...
             catpilot_toggles: SimpleNamespace, catpilotCarState: capnp._DynamicStructReader,
             low_speed_override: bool = True) -> dict[str, Any]:
  # Determine leads, this is where the essential logic happens
  if len(tracks) > 0 and ready and lead_msg.prob > catpilot_toggles.lead_detection_probability:
  ...
  elif (track is None) and ready and (lead_msg.prob > catpilot_toggles.lead_detection_probability):
  ...
  if 'dRel' in lead_dict:
    lead_dict['dRel'] -= catpilot_toggles.increased_stopped_distance if not catpilotCarState.trafficModeEnabled else 0
...
class RadarD:
  def __init__(self, radar_ts: float, delay: int = 0):
    ...
    self.classic_model = self.catpilot_toggles.classic_model
  ...
  def update(self, sm: messaging.SubMaster, rr):
    ...
    if len(leads_v3) > 1:
      self.radar_state.leadOne = get_lead(self.v_ego, self.ready, self.tracks, leads_v3[0], model_v_ego, sm['modelV2'], sm['carState'].standstill, self.catpilot_toggles, sm['catpilotCarState'], low_speed_override=True)
      self.radar_state.leadTwo = get_lead(self.v_ego, self.ready, self.tracks, leads_v3[1], model_v_ego, sm['modelV2'], sm['carState'].standstill, self.catpilot_toggles, sm['catpilotCarState'], low_speed_override=False)

    if self.catpilot_toggles.adjacent_lead_tracking and self.ready:
...
def main():
  ...
  sm = messaging.SubMaster(['modelV2', 'carState', 'catpilotCarState', 'catpilotPlan'], frequency=int(1./DT_CTRL))
