...
class Controls:
  def __init__(self, CI=None):
    ...
    if CI is None:
      ...
      with custom.CatPilotCarParams.from_bytes(self.params.get("CatPilotCarParamsPersistent", block=True)) as fpmsg:
    ...
    self.sm = messaging.SubMaster(['deviceState', 'pandaStates', 'peripheralState', 'modelV2', 'liveCalibration',
                                   ...
                                   'testJoystick', 'catpilotCarState', 'catpilotPlan'] + self.camera_packets + self.sensor_packets,
    ...
    self.use_old_long = self.catpilot_toggles.old_long_api
  ...
  def update_events(self, CS):
    ...
    if self.sm['modelV2'].meta.laneChangeState == LaneChangeState.preLaneChange:
      ...
      if (CS.leftBlindspot and direction == LaneChangeDirection.left) or \
        ...
        if self.catpilot_toggles.loud_blindspot_alert:
          self.events.add(EventName.laneChangeBlockedLoud)
        else:
          self.events.add(EventName.laneChangeBlocked)
      else:
        if direction == LaneChangeDirection.left:
          if self.sm['catpilotPlan'].laneWidthLeft >= self.catpilot_toggles.lane_detection_width:
            self.events.add(EventName.preLaneChangeLeft)
          else:
            self.events.add(EventName.noLaneAvailable)
        else:
          if self.sm['catpilotPlan'].laneWidthRight >= self.catpilot_toggles.lane_detection_width:
    ...
    elif not CS.canValid and not self.catpilot_toggles.force_onroad:
    ...
    # Add CatPilot events
    self.events.add_from_msg(self.sm['catpilotPlan'].catpilotEvents)

    if self.catpilot_toggles.block_user:
  ...
  def state_transition(self, CS):
    ...
    self.v_cruise_helper.update_v_cruise(CS, self.enabled, self.is_metric, self.sm['catpilotPlan'].speedLimitChanged, self.catpilot_toggles)
  ...
  def state_control(self, CS):
    ...
    if self.CP.lateralTuning.which() == 'torque':
      ...
      if self.sm.all_checks(['liveTorqueParameters']) and (torque_params.useParams or self.catpilot_toggles.force_auto_tune):
        self.LaC.update_live_torque_params(torque_params.latAccelFactorFiltered, torque_params.latAccelOffsetFiltered,
                                           torque_params.frictionCoefficientFiltered)

      if self.sm.updated['liveDelay'] and (self.catpilot_toggles.nnff or self.catpilot_toggles.nnff_lite):
    ...
    if not self.joystick_mode:
      ...
      if self.catpilot_toggles.sport_plus and (self.sm["catpilotCarState"].sportGear or not self.catpilot_toggles.map_acceleration):
        pid_accel_limits = (pid_accel_limits[0], get_max_allowed_accel(CS.vEgo))

      if self.use_old_long:
        t_since_plan = (self.sm.frame - self.sm.recv_frame['longitudinalPlan']) * DT_CTRL
        actuators.accel = min(self.LoC.update_old_long(CC.longActive, CS, long_plan, pid_accel_limits, t_since_plan, self.catpilot_toggles), self.catpilot_toggles.max_desired_acceleration)
      else:
        actuators.accel = min(self.LoC.update(CC.longActive, CS, long_plan.aTarget, long_plan.shouldStop, pid_accel_limits, self.catpilot_toggles), self.catpilot_toggles.max_desired_acceleration)
      ...
      actuators.steer, actuators.steeringAngleDeg, lac_log = self.LaC.update(CC.latActive, CS, self.VM, lp,
                                                                             ...
                                                                             model_data=self.sm['modelV2'], catpilot_toggles=self.catpilot_toggles)
    else:
    ...
    if lac_log.active and not recent_steer_pressed and not self.CP.notCar:
      if self.CP.lateralTuning.which() == 'torque' and not self.joystick_mode:
        ...
        if undershooting and turning and good_speed and max_torque:
          lac_log.active and self.events.add(EventName.goatSteerSaturated if self.catpilot_toggles.goat_scream_alert else EventName.steerSaturated)
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      if self.catpilot_toggles.personality_profile_via_distance:
        ...
        distance_pressed &= not (self.sm['catpilotCarState'].distanceLongPressed or self.sm['catpilotCarState'].distanceVeryLongPressed)
      if self.catpilot_toggles.personality_profile_via_distance_long:
        distance_pressed |= self.sm['catpilotCarState'].distanceLongPressed
      if self.catpilot_toggles.personality_profile_via_distance_very_long:
        distance_pressed |= self.sm['catpilotCarState'].distanceVeryLongPressed
      if self.catpilot_toggles.personality_profile_via_lkas:
        distance_pressed |= any(be.pressed and be.type == CatPilotButtonType.lkas for be in CS.buttonEvents)
    ...
    self.update_catpilot_variables(CS)

    return CC, lac_log

  def update_catpilot_variables(self, CS):
    if self.catpilot_toggles.conditional_experimental_mode or self.catpilot_toggles.slc_fallback_experimental_mode:
      self.experimental_mode = self.sm['catpilotPlan'].experimentalMode

    # Update CatPilot variables
    if self.sm['catpilotPlan'].togglesUpdated:
      self.catpilot_toggles = get_catpilot_toggles()
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    alerts = self.events.create_alerts(self.current_alert_types, [self.CP, CS, self.sm, self.is_metric, self.soft_disable_timer, self.catpilot_toggles])
    ...
    force_decel = (self.sm['driverMonitoringState'].awarenessStatus < 0.) or \
                  ...
                  self.sm['catpilotCarState'].forceCoast
