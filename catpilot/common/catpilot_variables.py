...
RESOURCES_REPO = "CatAi/CatPilot-Resources"
...
TORQUE_NN_MODEL_PATH = Path(BASEDIR) / "catpilot/assets/nnff_models"
...
def get_catpilot_toggles(block=True):
  return SimpleNamespace(**json.loads(params_memory.get("CatPilotToggles", block=block) or "{}"))

def update_catpilot_toggles():
  params_memory.put_bool("CatPilotTogglesUpdated", True)

catpilot_default_params: list[tuple[str, str | bytes, int, str]] = [
  ...
  ("CustomAlerts", "1", 0, "0"),
  ("CustomColors", "cat", 0, "stock"),
  ("CustomCruise", "1", 2, "1"),
  ("CustomCruiseLong", "5", 2, "5"),
  ("CustomDistanceIcons", "stock", 0, "stock"),
  ("CustomIcons", "cat-animated", 0, "stock"),
  ("CustomPersonalities", "0", 2, "0"),
  ("CustomSignals", "cat", 0, "stock"),
  ("CustomSounds", "cat", 0, "stock"),
  ("CustomUI", "1", 1, "0"),
  ...
  ("SshEnabled", "0", 0, "0"),
  ("StartupMessageBottom", "Human-tested, cat-approved 🐸", 0, "Always keep hands on wheel and eyes on road"),
...
class CatPilotVariables:
  def __init__(self):
    self.catpilot_toggles = get_catpilot_toggles(block=False)
    self.tuning_levels = {key: lvl for key, _, lvl, _ in catpilot_default_params + misc_tuning_levels}

    short_branch = get_build_metadata().channel
    self.development_branch = short_branch == "CatPilot-Development"
    self.release_branch = short_branch == "CatPilot"
    self.staging_branch = short_branch == "CatPilot-Staging"
    self.testing_branch = short_branch == "CatPilot-Testing"
    self.vetting_branch = short_branch == "CatPilot-Vetting"

    default = params_default
    level = self.tuning_levels
    toggle = self.catpilot_toggles

    toggle.cats_go_moo = Path("/persist/catsgomoo.py").is_file()
    toggle.block_user = (self.development_branch or short_branch == "MAKE-PRS-HERE" or self.vetting_branch) and not toggle.cats_go_moo
    ...
    for k, v, _, _ in catpilot_default_params:
      params_default.put(k, v)

    params_memory.put("CatPilotTuningLevels", json.dumps(self.tuning_levels))
  ...
  def update(self, holiday_theme, started, boot_run=False):
    ...
    fpmsg_bytes = params.get("CatPilotCarParams" if started else "CatPilotCarParamsPersistent", block=started)
    if fpmsg_bytes:
      with custom.CatPilotCarParams.from_bytes(fpmsg_bytes) as fpcp_reader:
        FPCP = fpcp_reader.as_builder()
    else:
      CarInterface, _, _ = interfaces[MOCK.MOCK]
      FPCP = CarInterface.get_catpilot_params(MOCK.MOCK, gen_empty_fingerprint(), [], toggle)
    ...
    toggle.has_zss = toggle.car_make == "toyota" and bool(FPCP.fpFlags & ToyotaCatPilotFlags.ZSS.value)
    ...
    params_memory.put("CatPilotToggles", json.dumps(toggle.__dict__))
    params_memory.remove("CatPilotTogglesUpdated")
