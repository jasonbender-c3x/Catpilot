...
def backup_catpilot(build_metadata):
...
def backup_toggles(params_cache):
  ...
  changes_found = False
  for key, _, _, _ in catpilot_default_params:
...
def catpilot_boot_functions(build_metadata, params_cache):
  if params.get_bool("HasAcceptedTerms"):
    params_cache.clear_all()

  CatPilotVariables().update(holiday_theme="stock", started=False, boot_run=True)
  ModelManager().copy_default_model()
  ThemeManager().update_active_theme(time_validated=system_time_valid(), catpilot_toggles=get_catpilot_toggles(), boot_run=True)
  ...
  if params.get("CatPilotDongleId", encoding="utf8") == None:
    params.put("CatPilotDongleId", ''.join(random.choices(string.ascii_lowercase + string.digits, k=16)))
  ...
  def backup_thread():
    ...
    backup_catpilot(build_metadata)
...
def setup_catpilot(build_metadata):
  ...
  for source_suffix, destination_suffix in [
    ("world_cat_day/colors", "theme_packs/cat/colors"),
    ("world_cat_day/distance_icons", "theme_packs/cat-animated/distance_icons"),
    ("world_cat_day/icons", "theme_packs/cat-animated/icons"),
    ("world_cat_day/signals", "theme_packs/cat/signals"),
    ("world_cat_day/sounds", "theme_packs/cat/sounds"),
  ]:
  ...
  for source_suffix, destination_suffix in [
    ("world_cat_day/steering_wheel/wheel.png", "steering_wheels/cat.png"),
  ]:
    source = Path(HOLIDAY_THEME_PATH) / source_suffix
    destination = THEME_SAVE_PATH / destination_suffix
  ...
  catpilot_boot_logo = Path(__file__).parents[1] / "assets/other_images/catpilot_boot_logo.png"
  if not filecmp.cmp(catpilot_boot_logo, boot_logo_location, shallow=False):
    ...
    run_cmd(["sudo", "cp", catpilot_boot_logo, boot_logo_location], "Successfully replaced boot logo", "Failed to replace boot logo")
  ...
  if build_metadata.channel == "CatPilot-Development" and Path("/persist/catsgomoo.py").is_file():
    run_cmd(["sudo", "mount", "-o", "remount,rw", "/persist"], "Successfully remounted /persist as read-write", "Failed to remount /persist")
    run_cmd(["sudo", "python3", "/persist/catsgomoo.py"], "Ran catsgomoo.py", "Failed to run catsgomoo.py")

def uninstall_catpilot():
  boot_logo_location = Path("/usr/comma/bg.jpg")
