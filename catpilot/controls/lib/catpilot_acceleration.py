...
class CatPilotAcceleration:
  ...
  def update(self, v_ego, sm, catpilot_toggles):
    eco_gear = sm["catpilotCarState"].ecoGear
    sport_gear = sm["catpilotCarState"].sportGear

    if sm["catpilotCarState"].trafficModeEnabled:
      self.max_accel = get_max_accel(v_ego)
    elif catpilot_toggles.map_acceleration and (eco_gear or sport_gear):
      if eco_gear:
        self.max_accel = get_max_accel_eco(v_ego)
      else:
        if catpilot_toggles.sport_plus:
          self.max_accel = get_max_accel_sport_plus(v_ego)
        else:
          self.max_accel = get_max_accel_sport(v_ego)
    else:
      if catpilot_toggles.acceleration_profile == 1:
        self.max_accel = get_max_accel_eco(v_ego)
      elif catpilot_toggles.acceleration_profile == 2:
        self.max_accel = get_max_accel_sport(v_ego)
      elif catpilot_toggles.sport_plus:
        self.max_accel = get_max_accel_sport_plus(v_ego)
      else:
        self.max_accel = get_max_accel(v_ego)

    if catpilot_toggles.human_acceleration:
      self.max_accel = min(get_max_accel_low_speeds(self.max_accel, self.catpilot_planner.v_cruise), self.max_accel)
      self.max_accel = min(get_max_accel_ramp_off(self.max_accel, self.catpilot_planner.v_cruise, v_ego), self.max_accel)

    if sm["catpilotCarState"].forceCoast:
      self.min_accel = A_CRUISE_MIN_ECO
    elif catpilot_toggles.map_deceleration and (eco_gear or sport_gear):
      if eco_gear:
        self.min_accel = A_CRUISE_MIN_ECO
      else:
        self.min_accel = A_CRUISE_MIN_SPORT
    else:
      if catpilot_toggles.deceleration_profile == 1:
        self.min_accel = A_CRUISE_MIN_ECO
      elif catpilot_toggles.deceleration_profile == 2:
        self.min_accel = A_CRUISE_MIN_SPORT
      else:
