#!/usr/bin/env python3
# PFEIFER - SLC - Modified by CatAi for CatPilot
import calendar
import json
import math
import numpy as np
...
class SpeedLimitController:
  def __init__(self):
    ...
    self.session.headers.update({"User-Agent": "catpilot-mapbox-speed-limit-retriever/1.0 (https://github.com/CatAi/CatPilot)"})

  @property
  def experimental_mode(self):
    return self.catpilot_toggles.slc_fallback_experimental_mode and self.target == 0

  @property
  def offset(self):
    if self.catpilot_toggles.is_metric:
      offset_map = [
        (0, 8.1, self.catpilot_toggles.speed_limit_offset1),      # 0–29 km/h
        (8.1, 13.6, self.catpilot_toggles.speed_limit_offset2),   # 30–49
        (13.6, 16.4, self.catpilot_toggles.speed_limit_offset3),  # 50–59
        (16.4, 21.9, self.catpilot_toggles.speed_limit_offset4),  # 60–79
        (21.9, 27.5, self.catpilot_toggles.speed_limit_offset5),  # 80–99
        (27.5, 33.1, self.catpilot_toggles.speed_limit_offset6),  # 100–119
        (33.1, 38.9, self.catpilot_toggles.speed_limit_offset7),  # 120–140
      ]
    else:
      offset_map = [
        (0, 11.2, self.catpilot_toggles.speed_limit_offset1),     # 0–24 mph
        (11.2, 15.2, self.catpilot_toggles.speed_limit_offset2),  # 25–34
        (15.2, 19.6, self.catpilot_toggles.speed_limit_offset3),  # 35–44
        (19.6, 24.1, self.catpilot_toggles.speed_limit_offset4),  # 45–54
        (24.1, 28.6, self.catpilot_toggles.speed_limit_offset5),  # 55–64
        (28.6, 33.1, self.catpilot_toggles.speed_limit_offset6),  # 65–74
        (33.1, 44.2, self.catpilot_toggles.speed_limit_offset7),  # 75–99
      ]
  ...
  def handle_limit_change(self, desired_source, desired_target, sm):
    ...
    speed_limit_accepted = (sm["catpilotCarState"].accelPressed and not sm["carControl"].cruiseControl.override) or params_memory.get_bool("SpeedLimitAccepted")
    speed_limit_denied = sm["catpilotCarState"].decelPressed or (self.speed_limit_changed_timer >= 30)
    ...
    elif desired_target < self.target and not self.catpilot_toggles.speed_limit_confirmation_lower:
      self.source = desired_source
      self.target = desired_target

    elif desired_target > self.target and not self.catpilot_toggles.speed_limit_confirmation_higher:
      self.source = desired_source
      self.target = desired_target
  ...
  def update_limits(self, dashboard_speed_limit, gps_position, navigation_speed_limit, v_cruise, v_ego, sm):
    ...
    if self.catpilot_toggles.speed_limit_priority_highest:
    ...
    elif self.catpilot_toggles.speed_limit_priority_lowest:
      desired_source = min(filtered_limits, key=filtered_limits.get, default="None")
      desired_target = filtered_limits.get(desired_source, 0)

    elif filtered_limits:
      for priority in [
        self.catpilot_toggles.speed_limit_priority1,
        self.catpilot_toggles.speed_limit_priority2,
        self.catpilot_toggles.speed_limit_priority3
      ]:
    ...
    if desired_target == 0 or self.target == 0:
      if self.mapbox_requests["total_requests"] < self.mapbox_requests["max_requests"] and self.catpilot_toggles.slc_mapbox_filler:
      ...
      if desired_target == 0 or self.target == 0:
        if self.denied_target != self.previous_target > 0 and self.catpilot_toggles.slc_fallback_previous_speed_limit:
          desired_source = self.previous_source
          desired_target = self.previous_target

          self.target = desired_target

        elif sm["controlsState"].enabled and self.catpilot_toggles.slc_fallback_set_speed:
          desired_source = "None"
  ...
  def update_override(self, v_cruise, v_cruise_diff, v_ego, v_ego_diff, sm):
    ...
    if self.override_slc:
      if self.catpilot_toggles.speed_limit_controller_override_manual:
      ...
      elif self.catpilot_toggles.speed_limit_controller_override_set_speed:
  ...
  def update_map_speed_limit(self, gps_position, v_ego):
    ...
    if self.next_speed_limit:
      ...
      if self.map_speed_limit < self.next_speed_limit:
        max_lookahead = self.catpilot_toggles.map_speed_lookahead_higher * v_ego
      elif self.map_speed_limit > self.next_speed_limit:
        max_lookahead = self.catpilot_toggles.map_speed_lookahead_lower * v_ego
      else:
        max_lookahead = 0
