...
class CatPilotEvents:
  def __init__(self, CatPilotPlanner):
    ...
    self.yourCatTriedToKillMe_played = False
  ...
  def update(self, v_cruise, sm, catpilot_toggles):
    ...
    if self.random_event_playing:
      ...
      if self.random_event_timer >= 5:
        update_wheel_image(catpilot_toggles.wheel_image, catpilot_toggles.current_holiday_theme, False)
    ...
    if self.catpilot_planner.catpilot_vcruise.forcing_stop:
      self.events.add(EventName.forcingStop)

    if not self.catpilot_planner.tracking_lead and sm["carState"].standstill and sm["carState"].gearShifter not in NON_DRIVING_GEARS and catpilot_toggles.green_light_alert:
      if not self.catpilot_planner.model_stopped and self.stopped_for_light:
        self.events.add(EventName.greenLight)

      self.stopped_for_light = self.catpilot_planner.cem.stop_light_detected
    else:
      self.stopped_for_light = False

    if not self.holidayActive_played and self.startup_seen and alerts_empty and catpilot_toggles.current_holiday_theme != "stock" and len(self.events) == 0:
      self.events.add(EventName.holidayActive)

      self.holidayActive_played = True

    if self.catpilot_planner.tracking_lead and sm["carState"].standstill and sm["carState"].gearShifter not in NON_DRIVING_GEARS and catpilot_toggles.lead_departing_alert:
      if self.tracking_lead_distance == 0:
        self.tracking_lead_distance = self.catpilot_planner.lead_one.dRel

      lead_departing = self.catpilot_planner.lead_one.dRel - self.tracking_lead_distance > 1
      lead_departing &= self.catpilot_planner.lead_one.vLead > 1
    ...
    if not self.torqueNNLoad_played and self.startup_seen and alerts_empty and len(self.events) == 0 and params.get("NNFFModelName", encoding="utf-8") is not None and catpilot_toggles.nnff:
      self.events.add(EventName.torqueNNLoad)

      self.torqueNNLoad_played = True

    if not self.random_event_playing and catpilot_toggles.random_events:
      ...
      if not self.dejaVuCurve_played and sm["carState"].vEgo > CRUISING_SPEED:
        if self.catpilot_planner.lateral_acceleration >= DEJA_VU_G_FORCE * ACCELERATION_DUE_TO_GRAVITY:
      ...
      if not self.vCruise69_played and 70 > max(sm["controlsState"].vCruise, sm["controlsState"].vCruiseCluster) * (1 if catpilot_toggles.is_metric else CV.KPH_TO_MPH) >= 69:
        ...
        if fcw_alert_match or stock_aeb_alert_match:
          ...
          if not self.yourCatTriedToKillMe_played:
            event_choices.append("yourCatTriedToKillMe")
          ...
          elif event_choice == "yourCatTriedToKillMe":
            self.events.add(EventName.yourCatTriedToKillMe)

            self.yourCatTriedToKillMe_played = True

          self.random_event_playing = True

      if not self.youveGotMail_played and sm["catpilotCarState"].alwaysOnLateralEnabled and not self.always_on_lateral_enabled_previously:
      ...
      self.always_on_lateral_enabled_previously = sm["catpilotCarState"].alwaysOnLateralEnabled

    if catpilot_toggles.speed_limit_changed_alert and self.catpilot_planner.catpilot_vcruise.slc.speed_limit_changed_timer == DT_MDL:
      self.events.add(EventName.speedLimitChanged)

    self.startup_seen |= sm["controlsState"].alertText1 == catpilot_toggles.startup_alert_top and sm["controlsState"].alertText2 == catpilot_toggles.startup_alert_bottom

    if sm["catpilotCarState"].trafficModeEnabled != self.previous_traffic_mode:
      if self.previous_traffic_mode:
        self.events.add(EventName.trafficModeInactive)
      else:
        self.events.add(EventName.trafficModeActive)
      self.previous_traffic_mode = sm["catpilotCarState"].trafficModeEnabled
