...
def get_city_center(latitude, longitude):
  try:
    with requests.Session() as session:
      ...
      session.headers.update({"User-Agent": "catpilot-city-center-checker/1.0 (https://github.com/CatAi/CatPilot)"})
...
def send_stats():
  try:
    ...
    if catpilot_toggles.cats_go_moo:
      return

    if catpilot_toggles.car_make == "mock":
      return
    ...
    catpilot_stats = json.loads(params.get("CatPilotStats") or "{}")
    ...
    theme_sources = [
      catpilot_toggles.icon_pack.replace("-animated", ""),
      catpilot_toggles.color_scheme,
      catpilot_toggles.distance_icons.replace("-animated", ""),
      catpilot_toggles.signal_icons.replace("-animated", ""),
      catpilot_toggles.sound_pack
    ]
    ...
    point = (Point("user_stats")
      .field("car_make", "GM" if catpilot_toggles.car_make == "gm" else catpilot_toggles.car_make.title())
      .field("car_model", catpilot_toggles.car_model)
      .field("city", city)
      .field("country", country)
      .field("device", HARDWARE.get_device_type())
      .field("driving_model", catpilot_toggles.model_name.replace("🗺️", "").replace("📡", "").replace("👀", "").replace("(Default)", "").strip())
      .field("event", 1)
      .field("catpilot_drives", params_tracking.get_int("CatPilotDrives"))
      .field("catpilot_hours", params_tracking.get_int("CatPilotMinutes") / 60)
      ...
      .field("rainbow_path", catpilot_toggles.rainbow_path)
      .field("random_events", catpilot_toggles.random_events)
      .field("state", state)
      .field("theme", selected_theme.title())
      .field("total_aol_seconds", float(catpilot_stats.get("TotalAOLTime", 0)))
      .field("total_lateral_seconds", float(catpilot_stats.get("TotalLateralTime", 0)))
      .field("total_longitudinal_seconds", float(catpilot_stats.get("TotalLongitudinalTime", 0)))
    ...
    print("Successfully sent CatPilot stats!")
  except Exception as exception:
    print(f"Failed to send CatPilot stats: {exception}")
