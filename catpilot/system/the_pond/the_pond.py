...
def setup(app):
  ...
  def get_stats():
    ...
    if short_branch == "CatPilot-Development":
      env = "Development"
    elif build_metadata.release_channel:
      env = "Release"
    elif short_branch == "CatPilot-Testing":
      env = "Testing"
  ...
  def restore_toggle_values():
    ...
    allowed_keys = {key for key, _, _, _ in catpilot_default_params if key not in EXCLUDED_KEYS}
