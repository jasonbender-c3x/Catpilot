...
class MapSpeedLogger:
  def __init__(self):
    ...
    self.session.headers.update({"User-Agent": "catpilot-map-speed-logger/1.0 (https://github.com/CatAi/CatPilot)"})

    self.sm = messaging.SubMaster(["deviceState", "catpilotCarState", "catpilotNavigation", "catpilotPlan", "liveLocationKalman", "modelV2"])
  ...
  def get_speed_limit_source(self):
    sources = [
      (self.sm["catpilotNavigation"].navigationSpeedLimit, "NOO"),
      (self.sm["catpilotPlan"].slcMapboxSpeedLimit, "Mapbox"),
      (self.sm["catpilotCarState"].dashboardSpeedLimit, "Dashboard"),
    ]
