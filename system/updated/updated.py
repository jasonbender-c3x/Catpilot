...
def finalize_update(manual_update_requested, params, catpilot_toggles) -> None:
  """Take the current OverlayFS merged view and finalize a copy outside of
  OverlayFS, ready to be swapped-in at BASEDIR. Copy using shutil.copytree"""

  while params_memory.get_bool("IsOnroad") and not manual_update_requested and not catpilot_toggles.cats_go_moo:
    time.sleep(5)
...
class Updater:
  ...
  def set_params(self, update_success: bool, failed_count: int, exception: str | None, catpilot_toggles: None) -> None:
  ...
  def fetch_update(self, manual_update_requested, catpilot_toggles) -> None:
    ...
    if params_memory.get_bool("IsOnroad") and not manual_update_requested and not catpilot_toggles.cats_go_moo:
      self.params.put("UpdaterState", "waiting for vehicle to go offroad...")
    finalize_update(manual_update_requested, self.params, catpilot_toggles)
  ...
  with open(LOCK_FILE, 'w') as ov_lock_fd:
    ...
    while True:
      ...
      try:
        ...
        updater.set_params(False, update_failed_count, exception, catpilot_toggles)
        ...
        if manual_update_requested or catpilot_toggles.automatic_updates:
          if params.get_bool("NetworkMetered") and not timed_out and not user_requested_fetch:
            cloudlog.info("skipping fetch, connection metered")
          elif wait_helper.user_request == UserRequest.CHECK:
            cloudlog.info("skipping fetch, only checking")
          else:
            updater.fetch_update(manual_update_requested, catpilot_toggles)
      ...
      try:
        ...
        updater.set_params(update_successful, update_failed_count, exception, catpilot_toggles)
      except Exception:
