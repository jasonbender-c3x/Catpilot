...
def hardware_thread(end_event, hw_queue) -> None:
  pm = messaging.PubMaster(['deviceState', 'catpilotDeviceState'])
  sm = messaging.SubMaster(["peripheralState", "gpsLocationExternal", "controlsState", "pandaStates", "catpilotPlan"], poll="pandaStates")
  ...
  while not end_event.is_set():
    ...
    if catpilot_toggles.increase_thermal_limits:
    ...
    should_start |= catpilot_toggles.force_onroad
    should_start &= not catpilot_toggles.force_offroad
    ...
    if power_monitor.should_shutdown(onroad_conditions["ignition"], in_car, off_ts, started_seen, catpilot_toggles):
    ...
    fpmsg = messaging.new_message('catpilotDeviceState')

    fpmsg.catpilotDeviceState.freeSpace = round(get_available_bytes(default=32.0 * (2 ** 30)) / (2 ** 30))
    fpmsg.catpilotDeviceState.usedSpace = round(get_used_bytes(default=0.0) / (2 ** 30))

    pm.send("catpilotDeviceState", fpmsg)
